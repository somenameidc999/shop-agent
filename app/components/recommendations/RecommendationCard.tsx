...
export interface Recommendation {
  ...
  readonly category: "inventory" | "customer" | "reporting" | "sync" | "marketing";
  readonly priority: "low" | "medium" | "high";
  readonly status: "pending" | "in_progress" | "completed" | "failed";
  readonly mcpServersUsed: readonly string[];
  readonly actionType: "chat_handoff" | "direct_execution";
...
interface RecommendationCardProps {
  ...
  readonly onExecute: (recommendation: Recommendation) => void;
...
const STATUS_CONFIG: Record<
  ...
  { color: string; bg: string; label: string }
> = {
  pending: { color: "#64748B", bg: "#F1F5F9", label: "Ready to execute" },
  in_progress: { color: "#2563EB", bg: "#EFF6FF", label: "Executing..." },
...
function getServerIcon(name: string): string {
  return SERVER_DISPLAY[name]?.icon ?? "apps";
}

const DEFAULT_NEXT_STEPS: Record<Recommendation["category"], string[]> = {
  inventory: [
...
export function RecommendationCard({
  ...
  const nextSteps =
    recommendation.nextSteps && recommendation.nextSteps.length > 0
      ...
      : DEFAULT_NEXT_STEPS[recommendation.category];
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {/* Data sources */}
        {recommendation.mcpServersUsed.length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {recommendation.mcpServersUsed.map((server) => (
              <div
                key={server}
                style={{
                  display: "inline-flex",
        ...
        >
          ...
          {expanded && (
            <div style={{ marginTop: 12 }}>
                ...
                {nextSteps.map((step, i) => (
                  ...
                  >
                    <span
                      style={{
                        color:
                          isCompleted && i < nextSteps.length
                            ? "#16A34A"
                            : "inherit",
                        textDecoration:
                          isCompleted ? "none" : "none",
                      }}
                    >
                      {step}
                    </span>
              ...
              {recommendation.actionType === "chat_handoff" && (
                <div
                  style={{
                    marginTop: 12,
                    padding: "10px 14px",
                ...
                >
                  <s-icon type="chat" />
                  This action will open a chat session with {AGENT_NAME} to walk you through it.
                </div>
              )}
            </div>
          )}
